...
//封闭环里面的图元应该是有序的排列，并选择其中一个图元作为封闭环的起点
...
typedef struct GeomCloseHEAD
{
	...
	struct GeomCloseHEAD*FirstInsideGCloseNode;//指向封闭环内部嵌套的第一个小封闭环头结点（指向第一个子封闭环）
	struct GeomCloseHEAD*prevOutsideGCloseNode;//由第一个子封闭环指向前一个父封闭环的头结点（指向第一个父封闭环）
