...
typedef struct NestResultDataNode//存储一张排样结果图的所有的图元的单向链表。
{
...
struct DXFProcData{
	...
	int BatchHEAD_ID;//初始化时候，还没有批次
	int NestResult_ID;//初始化，还没有图纸导入
	int GEOMCLOSE_ID;//初始化，还没有封闭环
	BatchHEAD* m_pBatchHead;//未指向任何地方
	NestResultDataNode* m_pNestrsltdtND;//每一个切割平面图（800*800）的信息。包括了所有封闭环
	GeomCloseHEAD* m_pGeomclsHead;
	bool m_IfDataDisposed;
	bool m_NewDxf;//新图纸
};
